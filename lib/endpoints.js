...
export default function getEndpoint(endpoint, pathParams) {
  switch (endpoint) {
    ...
    case 'headshot':
      ...
      else return `https://img.mlbstatic.com/mlb-photos/image/upload/d_people:generic:headshot:83:current.png,w_${pathParams.width},q_auto:best/v1/people/${pathParams.playerId}/headshot/83/current`
    case 'standings':
      return `https://statsapi.mlb.com/api/v1/standings?`
    case 'league':
      if (!('leagueId' in pathParams)) throw new Error('Missing key leagueId')
      else return `https://statsapi.mlb.com/api/v1/league/${pathParams.leagueId}?`
  }
}