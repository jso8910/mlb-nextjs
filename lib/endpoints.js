...
export default function getEndpoint(endpoint, pathParams) {
  switch (endpoint) {
    ...
    case 'headshot':
      if (!('playerId' in pathParams) && !('width' in pathParams)) throw new Error('Missing key playerId or width')
      else return `https://img.mlbstatic.com/mlb-photos/image/upload/d_people:generic:headshot:83:current.png,w_${width},q_auto:best/v1/people/${playerId}/headshot/83/current`
  }
}